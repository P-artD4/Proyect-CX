...
# Base de Datos en Google Sheets (tiempo real) + Modelo + Alertas
...
# ════════════════════════════════════════════════════════════
# CONEXIÓN A GOOGLE SHEETS — Base de datos en tiempo real
# ════════════════════════════════════════════════════════════
GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/d/1fr63q7FIJ-Ra44z8bCG5bw5hayYYIAKZIgmlHw786PM/edit"
...
def conectar_google_sheets():
    """Conecta usando las credenciales guardadas en Streamlit Secrets."""
...
def leer_hoja_sheets(nombre_hoja):
    """Lee una pestaña específica del Google Sheet como DataFrame."""
    cliente = conectar_google_sheets()
    sheet = cliente.open_by_url(GOOGLE_SHEET_URL)
...
# ════════════════════════════════════════════════════════════
# CAPA 1 — BASE DE DATOS: Lectura en tiempo real desde Sheets
# ════════════════════════════════════════════════════════════
HOJAS_REQUERIDAS = {
...
def cargar_datos_sheets():
    """Lee las 5 pestañas del Google Sheet en tiempo real."""
    try:
        cliente = conectar_google_sheets()
        sheet = cliente.open_by_url(GOOGLE_SHEET_URL)
        nombres_disponibles = [ws.title for ws in sheet.worksheets()]
    except Exception as e:
        return None, ['CONEXIÓN'], [], str(e)

    tablas = {}
    faltantes = []
    for clave, posibles in HOJAS_REQUERIDAS.items():
        encontrada = next((n for n in posibles if n in nombres_disponibles), None)
        if encontrada is None:
            faltantes.append(clave)
            continue
        df = leer_hoja_sheets(encontrada)
    ...
    if faltantes:
        return None, faltantes, nombres_disponibles, None
    return tablas, [], nombres_disponibles, None
...
def construir_variables(_tablas):
    df_clientes      = _tablas['Clientes'].replace('', np.nan)
    df_interacciones = _tablas['Interacciones'].replace('', np.nan)
    df_encuestas      = _tablas['Encuestas'].replace('', np.nan)
    df_preguntas      = _tablas['Preguntas'].replace('', np.nan)
    df_respuestas     = _tablas['Respuestas'].replace('', np.nan)
    ...
    if 'Antiguedad_Anios' in dm.columns:
        dm['Antiguedad_Anios'] = pd.to_numeric(dm['Antiguedad_Anios'], errors='coerce')
    ...
    # Forzar todas las columnas numéricas potenciales antes del fillna
    posibles_numericas = ['NPS_Promedio','NPS_Minimo','NPS_Maximo','NPS_Ultimo','NPS_Std',
                          ...
                          'Tasa_Respuesta','Antiguedad_Anios']
...
def ejecutar_modelo(_dm, _features):
    ...
    # Forzar conversión numérica — Google Sheets puede devolver
    # números como texto o celdas vacías como string ''
    for f in _features:
        dm[f] = pd.to_numeric(dm[f], errors='coerce')
    dm[_features] = dm[_features].fillna(dm[_features].median(numeric_only=True))
    dm[_features] = dm[_features].fillna(0)

    dm['Churn'] = pd.to_numeric(dm['Churn'], errors='coerce').fillna(0).astype(int)

    X = dm[_features].values.astype(float)
    y = dm['Churn'].values.astype(float)
...
with st.sidebar:
    ...
    st.markdown("""
    <div style="background:#1A2E4A; border-radius:10px; padding:10px 12px; margin-bottom:14px;">
        <div style="font-size:11px; color:#94A3B8;">🔗 Conectado a</div>
        <div style="font-size:12.5px; color:#E2E8F0; font-weight:600;">Google Sheets — Data Warehouse CX</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.caption("Los datos se sincronizan automáticamente cada 30 segundos, o al recargar la página.")
...
with st.spinner("Conectando con Google Sheets..."):
    tablas, faltantes, hojas_disponibles, error_conexion = cargar_datos_sheets()
...
if error_conexion:
    ...
    """, unsafe_allow_html=True)
    st.error(f"❌ No se pudo conectar con Google Sheets: {error_conexion}")
    st.info("Verifica que el archivo de credenciales esté configurado en "
            "Streamlit Secrets y que el Sheet esté compartido con la cuenta de servicio.")
    st.stop()

if faltantes:
    st.error(f"❌ No se encontraron las siguientes pestañas en el Sheet: {faltantes}")
    st.write(f"Pestañas disponibles: {hojas_disponibles}")
    st.stop()
